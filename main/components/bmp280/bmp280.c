...
static esp_err_t bmp280_read_uint16_le(bmp280_handle_t sensor, uint8_t addr,
                                       uint16_t *data) {
...
unsigned int bmp280_getconfig(bmp280_handle_t sensor) {
  bmp280_dev_t *sens = (bmp280_dev_t *)sensor;
  return (sens->config_t.t_sb << 5) | (sens->config_t.filter << 3) |
         sens->config_t.spi3w_en;
}

unsigned int bmp280_getctrl_meas(bmp280_handle_t sensor) {
  bmp280_dev_t *sens = (bmp280_dev_t *)sensor;
  return (sens->ctrl_meas_t.osrs_t << 5) | (sens->ctrl_meas_t.osrs_p << 3) |
         sens->ctrl_meas_t.mode;
}

bool bmp280_is_reading_calibration(bmp280_handle_t sensor) {
  uint8_t rstatus = 0;
  bmp280_dev_t *sens = (bmp280_dev_t *)sensor;
  if (i2c_bus_read_byte(sens->i2c_dev, BMP280_REGISTER_STATUS, &rstatus) !=
      ESP_OK) {
    return false;
  }
...
esp_err_t bmp280_read_coefficients(bmp280_handle_t sensor) {
  uint16_t data16 = 0;
  bmp280_dev_t *sens = (bmp280_dev_t *)sensor;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_T1, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_t1 = data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_T2, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_t2 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_T3, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_t3 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P1, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p1 = data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P2, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p2 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P3, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p3 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P4, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p4 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P5, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p5 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P6, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p6 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P7, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p7 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P8, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  sens->data_t.dig_p8 = (int16_t)data16;
  if (bmp280_read_uint16_le(sensor, BMP280_REGISTER_DIG_P9, &data16) !=
      ESP_OK) {
    return ESP_FAIL;
  }
...
esp_err_t bmp280_set_sampling(bmp280_handle_t sensor, bmp280_sensor_mode mode,
  ...
  if (i2c_bus_write_byte(sens->i2c_dev, BMP280_REGISTER_CONFIG,
                         bmp280_getconfig(sensor)) != ESP_OK) {
    return ESP_FAIL;
  }
  if (i2c_bus_write_byte(sens->i2c_dev, BMP280_REGISTER_CONTROL,
                         bmp280_getctrl_meas(sensor)) != ESP_OK) {
    return ESP_FAIL;
  }
  return ESP_OK;
}
...
esp_err_t bmp280_default_init(bmp280_handle_t sensor) {
  ...
  if (i2c_bus_write_byte(sens->i2c_dev, BMP280_REGISTER_SOFTRESET, 0xB6) !=
      ESP_OK) {
    return ESP_FAIL;
  }
  ...
  if (bmp280_read_coefficients(sensor) !=
      ESP_OK) { // read trimming parameters, see DS 4.2.2
    return ESP_FAIL;
  }
  if (bmp280_set_sampling(sensor, BMP280_MODE_NORMAL, BMP280_SAMPLING_X16,
                          BMP280_SAMPLING_X16, BMP280_FILTER_OFF,
...
esp_err_t bmp280_take_forced_measurement(bmp280_handle_t sensor) {
  ...
  if (sens->ctrl_meas_t.mode == BMP280_MODE_FORCED) {
    ...
    if (i2c_bus_read_byte(sens->i2c_dev, BMP280_REGISTER_STATUS, &data) !=
        ESP_OK) {
      return ESP_FAIL;
    }
    while (data & 0x08) {
...
esp_err_t bmp280_read_temperature(bmp280_handle_t sensor, float *temperature) {
  ...
  uint8_t data[3] = {0};
  bmp280_dev_t *sens = (bmp280_dev_t *)sensor;
  if (i2c_bus_read_bytes(sens->i2c_dev, BMP280_REGISTER_TEMPDATA, 3, data) !=
      ESP_OK) {
    ESP_LOGE("BMP", "ERROR ON BUS READ");
    return ESP_FAIL;
  }
  ...
  var1 = ((((adc_T >> 3) - ((int32_t)sens->data_t.dig_t1 << 1))) *
          ((int32_t)sens->data_t.dig_t2)) >>
         11;

  var2 = (((((adc_T >> 4) - ((int32_t)sens->data_t.dig_t1)) *
            ((adc_T >> 4) - ((int32_t)sens->data_t.dig_t1))) >>
           12) *
          ((int32_t)sens->data_t.dig_t3)) >>
         14;
...
esp_err_t bmp280_read_pressure(bmp280_handle_t sensor, float *pressure) {
  ...
  if (i2c_bus_read_bytes(sens->i2c_dev, BMP280_REGISTER_PRESSUREDATA, 3,
                         data) != ESP_OK) {
    return ESP_FAIL;
  }
