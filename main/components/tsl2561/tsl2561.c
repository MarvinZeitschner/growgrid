...
esp_err_t tsl2561_init(tsl2561_handle_t *sensor,
  ...
  esp_err_t err = i2c_bus_write_byte(sensor->dev_handle,
                                     TSL2561_CMD | TSL2561_REG_CONTROL, data);
  if (err != ESP_OK)
    return err;

  uint8_t timing = sensor->gain | (sensor->integration_time & 0x03);
  return i2c_bus_write_byte(sensor->dev_handle,
...
esp_err_t tsl2561_read_channels(tsl2561_handle_t *sensor, uint16_t *ch0,
  ...
  uint8_t data0[2];
  esp_err_t err = i2c_bus_read_bytes(sensor->dev_handle, reg, 2, data0);
  *ch0 = (data0[1] << 8) | data0[0];

  reg = TSL2561_CMD_WORD | TSL2561_REG_DATA1LOW;
  uint8_t data1[2];
  err = i2c_bus_read_bytes(sensor->dev_handle, reg, 2, data1);
  if (err != ESP_OK)
    return err;
